...
public class ActivityAddKhachHang extends AppCompatActivity {
    ...
    public boolean check() {
        ...
        if (makh.isEmpty() || tenkh.isEmpty() || sdt.isEmpty() || diachi.isEmpty()) {
            ...
            return false;
        }else if (!makh.substring(0,1).toUpperCase().equals(makh.substring(0,1))) {
            Toast.makeText(this, "Kí tự đầu mã khách hàng phải in hoa", Toast.LENGTH_SHORT).show();
            return false;
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.menu_save:
                if (check()) {
                    String maKhang=ed_maKhachHang.getText().toString();
                    if (daoKhachHang.checkmaKH(maKhang)>0){
                        Toast.makeText(this, "Mã khách hàng đã tồn tại hệ thống", Toast.LENGTH_SHORT).show();
                        return false;
                    }else {
