...
public class ActivityAddHoaDonXuat extends AppCompatActivity {
    ...
    private void eventDialog() {
        ...
        ed_soluongHD_xuat.addTextChangedListener(new TextWatcher() {
            @Override
    ...
    public boolean check() {
        ...
        for (int i = 0; i < slNhaps.size(); i++) {
            if (slNhaps.get(i).mssp.equals(maSP)) {
                if (slNhaps.get(i).soluongnhap < Integer.parseInt(soluong)) {
                    Toast.makeText(this, "Số lượng sản phẩm còn: " + slNhaps.get(i).soluongnhap, Toast.LENGTH_SHORT).show();
                    return false;
                }
            }
        }
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        switch (item.getItemId()) {
            case R.id.menu_reset:
                ed_maHD_xuat.setText("");
                ed_ngayHD_xuat.setText("");
                ed_thanhtienHD_xuat.setText("");
                rdo_khongbaohanh_HD_xuat.setChecked(true);
                rdo_chuathanhtoan_HD.setChecked(true);
                return true;
            case R.id.menu_save:
                if (check()) {
                    String mahd = ed_maHD_xuat.getText().toString();
